			...
			<div id="contents">
				<div id="customer">
					...
					function pageBtn(page) {
						alert("현재 클릭한 페이지 번호 : "+ page);
						alert("카테고리 : " + $(".category").val() );
						alert("검색어 : " + $(".searchInput").val() );
						
						var category = $(".category").val();
						var searchword = $(".searchInput").val();
						var url = "../S03?page="+page+"&category="+category+"&searchword="+searchword;
						console.log(url);
						location.href= url;
					}					
					</script>	
					...
					<div class="btnAreaList">
						...
						<%for(int i=1;i<=10;i++){%>
						<%if(i==7){ %>
						<a onclick='pageBtn(<%= i %>)'><strong><%= i %></strong></a>
						<%} else {%>
						<a onclick='pageBtn(<%= i %>)'><%= i %></a>
						<%} %>
						<%}%>
