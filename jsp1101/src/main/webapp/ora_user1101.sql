...
and lower(name) like '%d%' --name 소문자로 변경후 찾기
and gender = 'Male'
and total>=250;

--David Austin이 속한 부서의 사람출력(사원번호,사원명,월급,부서번호,부서명,부서입력일,부서수정일,직급번호
select * from employees;

select department_id from employees
where emp_name='David Austin';

select employee_id, emp_name, salary, b.department_id, department_name, b.create_date, b.update_date, job_id 
from employees a, departments b
...
--직급별 최소월급이 5000 이상 직급의 사람들 중 부서번호가 50번이상인 사람들만 모두 출력
--사원번호, 사원명, 월급, 직급번호, 직급별 최소월급, 부서번호, 부서명 출력
...
--SA_MAN 직급을 가진 사원번호,사원명,직급번호,부서번호,부서명 출력

select employee_id,emp_name,job_id,a.department_id,department_name
from employees a, departments b
...
-- stunum,name,grade(학년),total,avg,grade(성적) 출력

select a.stunum,name,a.grade "학년",total,round(avg,2),c.grade "성적"
from stuinfo a, stuscore b, stugrade c
where a.stunum=b.stunum
and floor(avg) between low_score and high_score;
...
select substr(period,1,4), to_char( sum(loan_jan_amt),'999,999,999.9') from kor_loan_status
group by substr(period,1,4)
order by substr(period,1,4) asc;
...
order by a.department_id, employee_id;

-- maneger name Steven King인 사원 출력
select a.employee_id, a.emp_name, a.manager_id, b.emp_name
from employees a, employees b
where a.manager_id=b.employee_id
...
-- +부서번호, 부서명, 직급타이틀
select a.employee_id, a.emp_name, a.manager_id, b.emp_name, a.department_id, department_name,job_title
...
select a.employee_id, a.emp_name, a.manager_id,b.employee_id, b.emp_name
from employees a, employees b
where a.manager_id=b.employee_id;
