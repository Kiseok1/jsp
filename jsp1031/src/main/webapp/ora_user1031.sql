--7--
--부서별 최대연봉 최소연봉 평균연봉을 출력
select max(salary),min(salary) from employees;

select department_id,max(salary),min(salary),round(avg(salary),2) from employees
group by department_id
order by department_id;

-- 평균 부서별 연봉이 5000이상인 부서만 출력
select department_id,max(salary),min(salary),round(avg(salary),2) from employees
...
select employee_id, emp_name, departments.department_id, department_name, departments.manager_id, departments.create_date 
...
select * from employees, departments; -- (107 * 27)
 ...
 -- equi join : 공통의 컬럼으로 검색
select employees.department_id, department_name from employees, departments
where employees.department_id = departments.department_id;

select e.department_id, department_name from employees e, departments d
where e.department_id = d.department_id;

select * from jobs;

-- 사원번호, 사원이름, 직급, 직급의 최대월급과 최소월급 출력
select e.employee_id, e.emp_name, e.job_id, j.job_title, j.min_salary,j. max_salary
